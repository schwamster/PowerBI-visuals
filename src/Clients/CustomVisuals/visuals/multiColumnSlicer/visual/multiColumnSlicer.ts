...
module powerbi.visuals.samples {
    ...
    export interface MultiColumnSlicerSettings {
        general: {
            orientation: string;
            columns: number;
            rows: number;
    ...
    export var multiColumnSlicerProps = {
     ...
     module BorderStyle {
        ...
        export var type: IEnumType = createEnumType([
            { value: ROUNDED, displayName: BorderStyle.ROUNDED },
            { value: CUT, displayName: BorderStyle.CUT },
            { value: SQUARE, displayName: BorderStyle.SQUARE },
        ]);
    }
    ...
    module ShowDisabled {
        ...
        export var type: IEnumType = createEnumType([
            { value: INPLACE, displayName: ShowDisabled.INPLACE },
            { value: BOTTOM, displayName: ShowDisabled.BOTTOM },
            { value: HIDE, displayName: ShowDisabled.HIDE },
        ]);
    }
    ...
    export class MultiColumnSlicer implements IVisual {
        ...
        public static capabilities: VisualCapabilities = {
                ...
                general: {
                    ...
                    properties: {
                        ...
                        showDisabled: {
                            ...
                            type: { enumeration: ShowDisabled.type }
                        },
                        multiselect: {
            ...
            dataViewMappings: [{
                ...
                categorical: {
                    categories: {
                        for: { in: 'Category' },
                        dataReductionAlgorithm: { top: { count: 10 } }
                    },
        ...
        private slicerData: MultiColumnSlicerViewModel;
        private settings: MultiColumnSlicerSettings;
        private interactivityService: IInteractivityService;
        private behavior: MultiColumnSlicerWebBehavior;
        ...
        public static DefaultStyleProperties(): MultiColumnSlicerSettings {
            return {
                general: {
                    ...
                    rows: 0,
                    multiselect: true,
                    showDisabled: ShowDisabled.INPLACE,
                    selection: null,
        ...
        constructor(options?: MultiColumnSlicerConstructorOptions) {
            if (options) {
                if (options.behavior) {
            ...
            if (!this.behavior) {
                this.behavior = new MultiColumnSlicerWebBehavior();
            }
        }

        public static converter(dataView: DataView, localizedSelectAllText: string, searchText: string, interactivityService: IInteractivityService): MultiColumnSlicerViewModel {
            if (!dataView ||
            ...
            if (objects) {
                defaultSettings.general.orientation = DataViewObjects.getValue<string>(objects, multiColumnSlicerProps.general.orientation, defaultSettings.general.orientation);
                defaultSettings.general.columns = DataViewObjects.getValue<number>(objects, multiColumnSlicerProps.general.columns, defaultSettings.general.columns);
                defaultSettings.general.rows = DataViewObjects.getValue<number>(objects, multiColumnSlicerProps.general.rows, defaultSettings.general.rows);
                defaultSettings.general.multiselect = DataViewObjects.getValue<boolean>(objects, multiColumnSlicerProps.general.multiselect, defaultSettings.general.multiselect);
                defaultSettings.general.showDisabled = DataViewObjects.getValue<string>(objects, multiColumnSlicerProps.general.showDisabled, defaultSettings.general.showDisabled);
                defaultSettings.general.selection = DataViewObjects.getValue(dataView.metadata.objects, multiColumnSlicerProps.general.selection, defaultSettings.general.selection);
                defaultSettings.general.selfFilterEnabled = DataViewObjects.getValue<boolean>(objects, multiColumnSlicerProps.general.selfFilterEnabled, defaultSettings.general.selfFilterEnabled);

                defaultSettings.header.show = DataViewObjects.getValue<boolean>(objects, multiColumnSlicerProps.header.show, defaultSettings.header.show);
                defaultSettings.header.title = DataViewObjects.getValue<string>(objects, multiColumnSlicerProps.header.title, defaultSettings.header.title);
                defaultSettings.header.fontColor = DataViewObjects.getFillColor(objects, multiColumnSlicerProps.header.fontColor, defaultSettings.header.fontColor);
                defaultSettings.header.background = DataViewObjects.getFillColor(objects, multiColumnSlicerProps.header.background, defaultSettings.header.background);
                defaultSettings.header.textSize = DataViewObjects.getValue<number>(objects, multiColumnSlicerProps.header.textSize, defaultSettings.header.textSize);
                defaultSettings.header.outline = DataViewObjects.getValue<string>(objects, multiColumnSlicerProps.header.outline, defaultSettings.header.outline);
                defaultSettings.header.outlineColor = DataViewObjects.getFillColor(objects, multiColumnSlicerProps.header.outlineColor, defaultSettings.header.outlineColor);
                defaultSettings.header.outlineWeight = DataViewObjects.getValue<number>(objects, multiColumnSlicerProps.header.outlineWeight, defaultSettings.header.outlineWeight);

                defaultSettings.slicerText.textSize = DataViewObjects.getValue<number>(objects, multiColumnSlicerProps.rows.textSize, defaultSettings.slicerText.textSize);
                defaultSettings.slicerText.height = DataViewObjects.getValue<number>(objects, multiColumnSlicerProps.rows.height, defaultSettings.slicerText.height);
                defaultSettings.slicerText.width = DataViewObjects.getValue<number>(objects, multiColumnSlicerProps.rows.width, defaultSettings.slicerText.width);
                defaultSettings.slicerText.selectedColor = DataViewObjects.getFillColor(objects, multiColumnSlicerProps.rows.selectedColor, defaultSettings.slicerText.selectedColor);
                defaultSettings.slicerText.hoverColor = DataViewObjects.getFillColor(objects, multiColumnSlicerProps.rows.hoverColor, defaultSettings.slicerText.hoverColor);
                defaultSettings.slicerText.unselectedColor = DataViewObjects.getFillColor(objects, multiColumnSlicerProps.rows.unselectedColor, defaultSettings.slicerText.unselectedColor);
                defaultSettings.slicerText.disabledColor = DataViewObjects.getFillColor(objects, multiColumnSlicerProps.rows.disabledColor, defaultSettings.slicerText.disabledColor);
                defaultSettings.slicerText.background = DataViewObjects.getFillColor(objects, multiColumnSlicerProps.rows.background, defaultSettings.slicerText.background);
                defaultSettings.slicerText.transparency = DataViewObjects.getValue<number>(objects, multiColumnSlicerProps.rows.transparency, defaultSettings.slicerText.transparency);
                defaultSettings.slicerText.fontColor = DataViewObjects.getFillColor(objects, multiColumnSlicerProps.rows.fontColor, defaultSettings.slicerText.fontColor);
                defaultSettings.slicerText.outline = DataViewObjects.getValue<string>(objects, multiColumnSlicerProps.rows.outline, defaultSettings.slicerText.outline);
        ...
        public init(options: VisualInitOptions): void {
            ...
            this.hostServices.canSelect = MultiColumnSlicer.canSelect;
            this.settings = MultiColumnSlicer.DefaultStyleProperties();
        ...
        private updateInternal(resetScrollbarPosition: boolean) {
            ...
            var data = MultiColumnSlicer.converter(this.dataView, localizedSelectAllText, this.searchInput.val(), this.interactivityService);
            if (!data) {
            ...
            if (this.settings.general.showDisabled === ShowDisabled.BOTTOM) {
            ...
            } else if (this.settings.general.showDisabled === ShowDisabled.HIDE) {
                data.slicerDataPoints = data.slicerDataPoints.filter(x => x.selectable);
            }

            var height: number = this.settings.slicerText.height;
            if (height === 0) {
                var extraSpaceForCell = MultiColumnSlicer.cellTotalInnerPaddings + MultiColumnSlicer.cellTotalInnerBorders;
                var textProperties = MultiColumnSlicer.getChicletTextProperties(this.settings.slicerText.textSize);
            ...
            this.tableView
                ...
                (d: MultiColumnInfo) => $.inArray(d, data.slicerDataPoints),
         ...
         private initContainer() {
            var settings: MultiColumnSlicerSettings  = this.settings;
            ...
            var slicerContainer: D3.Selection = d3.select(this.element.get(0))
                .append('div')
                .classed(MultiColumnSlicer.Container.class, true);

            this.slicerHeader = slicerContainer
                .append('div')
                .classed(MultiColumnSlicer.Header.class, true);

            this.slicerHeader
                .append('span')
                .classed(MultiColumnSlicer.Clear.class, true)
                .attr('title', 'Clear');

            this.slicerHeader
                .append('div')
                .classed(MultiColumnSlicer.HeaderText.class, true)
                .style({
            ...
            this.slicerBody = slicerContainer
                .append('div').classed(MultiColumnSlicer.Body.class, true)
            ...
            var rowEnter = (rowSelection: D3.Selection) => {
                ...
                var listItemElement = rowSelection.append('li')
                    .classed(MultiColumnSlicer.ItemContainer.class, true)
                    .style({
                ...
                listItemElement.append('div')
                    ...
                    .append('span')
                    .classed(MultiColumnSlicer.LabelText.class, true)
                    .style({
            ...
            var rowUpdate = (rowSelection: D3.Selection) => {
                ...
                if (data && settings) {
                    ...
                    if (this.interactivityService && this.slicerBody) {
                        ...
                        var slicerItemContainers = slicerBody.selectAll(MultiColumnSlicer.ItemContainer.selector);
                        var slicerItemLabels = slicerBody.selectAll(MultiColumnSlicer.LabelText.selector);
                        var slicerItemInputs = slicerBody.selectAll(MultiColumnSlicer.Input.selector);
                        var slicerClear = this.slicerHeader.select(MultiColumnSlicer.Clear.selector);

                        var behaviorOptions: MultiColumnSlicerBehaviorOptions = {
                        ...
                        this.behavior.styleSlicerInputs(rowSelection.select(MultiColumnSlicer.ItemContainer.selector),
                            this.interactivityService.hasSelection());
                    }
                    else {
                        this.behavior.styleSlicerInputs(rowSelection.select(MultiColumnSlicer.ItemContainer.selector), false);
                    }
                }
            };
        ...
        public static getChicletTextProperties(textSize?: number): TextProperties {
            return <TextProperties>{
                fontFamily: MultiColumnSlicer.DefaultFontFamily,
                fontSize: PixelConverter.fromPoint(textSize || MultiColumnSlicer.DefaultFontSizeInPt),
            };
        }

        private getHeaderHeight(): number {
            return TextMeasurementService.estimateSvgTextHeight(
                MultiColumnSlicer.getChicletTextProperties(this.settings.header.textSize));
        }
        ...
        private getRowHeight(): number {
            ...
            return textSettings.height !== 0
                ? textSettings.height
                : TextMeasurementService.estimateSvgTextHeight(MultiColumnSlicer.getChicletTextProperties(textSettings.textSize));
        }
        ...
        public enumerateObjectInstances(options: EnumerateVisualObjectInstancesOptions): VisualObjectInstance[] {
            var data: MultiColumnSlicerViewModel = this.slicerData;
            if (!data) {
                return;
            }
         ...
         private enumerateHeader(data: MultiColumnSlicerViewModel): VisualObjectInstance[] {
            var slicerSettings: MultiColumnSlicerSettings = this.settings;
            return [{
                selector: null,
                objectName: 'header',
                properties: {
        ...
        private enumerateRows(data: MultiColumnSlicerViewModel): VisualObjectInstance[] {
            var slicerSettings: MultiColumnSlicerSettings = this.settings;
            return [{
                selector: null,
                objectName: 'rows',
        ...
        private enumerateGeneral(data: MultiColumnSlicerViewModel): VisualObjectInstance[] {
            var slicerSettings: MultiColumnSlicerSettings = this.settings;
    ...
    module MultiColumnSlicerChartConversion {
        export class MultiColumnSlicerConverter {
            ...
            public convert(): void {
                ...
                if (this.columns) {
                    ...
                    for(var columnIndex: number = 0; columnIndex < this.columns.length; columnIndex++){  
                        var category: DataViewCategoryColumn = this.columns[columnIndex];                     
                        ...
                        if (objects && objects.general && objects.general.filter) {
                            if (!categoryColumnRef)
                                return;
                            var filter = <SemanticFilter>objects.general.filter;
                            var scopeIds = SQExprConverter.asScopeIdsContainer(filter, categoryColumnRef);
                            if (scopeIds) {
                        ...
                        for (var idx = 0; idx < categoryValues.length; idx++) {
                            var selected = isCategoryColumnSelected(multiColumnSlicerProps.selectedPropertyIdentifier, category, idx);
                            if (selected != null) {
                        ...
                        for (var categoryIndex: number = 0, categoryCount = categoryValues.length; categoryIndex < categoryCount; categoryIndex++) {
                            ...
                            var categoryIsSelected = isCategoryColumnSelected(multiColumnSlicerProps.selectedPropertyIdentifier, category, categoryIndex);
    ...
    export interface MultiColumnSlicerBehaviorOptions {
        slicerItemContainers: D3.Selection;
        slicerItemLabels: D3.Selection;
        slicerItemInputs: D3.Selection;
        slicerClear: D3.Selection;
        dataPoints: MultiColumnInfo[];
        interactivityService: IInteractivityService;
        slicerSettings: MultiColumnSlicerSettings;
        isSelectionLoaded: boolean;
    }

    export class MultiColumnSlicerWebBehavior implements IInteractiveBehavior {
        private slicers: D3.Selection;
        private slicerItemLabels: D3.Selection;
        private slicerItemInputs: D3.Selection;
        private dataPoints: MultiColumnInfo[];
        private interactivityService: IInteractivityService;
        private slicerSettings: MultiColumnSlicerSettings;
        private options: MultiColumnSlicerBehaviorOptions;

        public bindEvents(options: MultiColumnSlicerBehaviorOptions, selectionHandler: ISelectionHandler): void {
            ...
            slicers.on("mouseover", (d: MultiColumnInfo) => {
                if (d.selectable) {
                    d.mouseOver = true;
                    d.mouseOut = false;
            ...
            slicers.on("mouseout", (d: MultiColumnInfo) => {
                if (d.selectable) {
                    d.mouseOver = false;
                    d.mouseOut = true;
                    this.renderMouseover();
            ...
            slicers.on("click", (d: MultiColumnInfo, index) => {
                if (!d.selectable) {
                    return;
                }
        ...
        public loadSelection(selectionHandler: ISelectionHandler): void {
            ...
            if (savedSelectionIds.length) {
                ...
                selectionHandler.persistSelectionFilter(multiColumnSlicerProps.filterPropertyIdentifier);
            }
        }
        ...
        public saveSelection(selectionHandler: ISelectionHandler): void {
            var filter: SemanticFilter = MultiColumnSlicerWebBehavior.getFilterFromSelectors(selectionHandler, this.interactivityService.isSelectionModeInverted());
        ...
        public styleSlicerInputs(slicers: D3.Selection, hasSelection: boolean) {
            ...
            var selectedItems = [];
            slicers.each(function (d: MultiColumnInfo) {
                // get selected items
